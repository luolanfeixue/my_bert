...
class OCRProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_data.csv")), "dev")

    def get_test_examples(self, data_dir):
        """Gets a collection of `InputExample`s for prediction."""
        raise self._create_examples(
            self._read_tsv(os.path.join(data_dir, "evaluationDataForA.csv")), "test")
...
def main(_):
  ...
  # 检查一下大小写的配置与所使用模型是否匹配
  tokenization.validate_case_matches_checkpoint(FLAGS.do_lower_case,FLAGS.init_checkpoint)
  if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict:
    raise ValueError("At least one of 'do_train','do_evel' or 'do_predict' must be True")
  
  # 读取bert 配置，配置了模型架构
  bert_config = modeling.BertConfig.from_json_file(FLAGS.bert_config_file)
  if FLAGS.max_seq_length > bert_config.max_position_embeddings:
    raise ValueError("Cannot use sequence length %d because the BERT model was only traied up to sequence length %d" %
...
# required parameters
flags.DEFINE_string('data_dir', '/home/hhl/code/my_bert/data/JDATA/', "The input data dir. Should contain the .tsv files (or other data files) for the task")
flags.DEFINE_string('task_name', 'OCR', 'the name of task to train')
...
flags.DEFINE_string('output_dir', './jdata_model', 'the output dir where the model checkpoints wile be written')
...
flags.DEFINE_bool('do_train',False,'do train')
flags.DEFINE_bool('do_eval', False,'do eval')
flags.DEFINE_bool('do_predict', True,'do predict')
